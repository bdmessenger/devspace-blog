            ...
            <ul className="flex pl-0 list-none my-2">
                ...
                {Array.from({length: numPages}, (_,i) => (
                    <Link key={i} href={`/blog/page/${i + 1}`}>
                        <li className={`relative block py-2 px-3 leading-tight ${currentPage === (i + 1) ? 'bg-gray-300' : 'bg-white'} border border-gray-300 text-gray-800 mr-1 hover:bg-gray-200 cursor-pointer`}>
                            {i + 1}
                        </li>
                    </Link>
                ))}
